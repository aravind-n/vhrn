...
func TestContractProcessRows(t *testing.T) {
	for _, row := range contractRows(t, "proxy-process-cases.tsv") {
		...
		if row[1] == "/__status" && !strings.Contains(recorder.Body.String(), `"mode":"enforce"`) {
...
func contractPublicHTTPRow(t *testing.T, row []string) {
	...
	if strings.HasPrefix(row[1], "https:") {
		px := httptest.NewServer(NewProxy(fakeChecker{verdict: Verdict{Allow: true}}, fakeDialer{target: "127.0.0.1:1"}, &fakeRecorder{}))
		defer px.Close()
...
func contractLocalHTTPRow(t *testing.T, row []string) {
	...
	defer cancel()
	request, err := http.NewRequestWithContext(ctx, http.MethodGet, origin.URL, nil)
	if err != nil {
		t.Fatal(err)
	}
