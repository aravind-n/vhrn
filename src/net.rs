...
mod tests {
    ...
    fn domain_policy_fixture_remains_proxy_compatible() {
            ...
            .filter(|row| !row.starts_with('#'))
        {
            ...
            if let ["entry", input, normalized, _, outcome] = fields.as_slice() {
                let result = normalize_domain(input);
                assert_eq!(result.is_ok(), *outcome == "accept", "{row}");
                if let Ok(value) = result {
                    assert_eq!(value, *normalized, "{row}");
                }
            }
        }
    }
