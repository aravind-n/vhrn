...
- [ ] Status: Not started
- Depends on: [Phase 11](phase-11-resources-shutdown.md) complete.
...
1. Audit the [contract coverage matrix](plan.md#contract-coverage-matrix) against named tests. Add only cross-feature black-box cases here; if a
   normative behavior lacks its owning phase's focused evidence, reopen that phase and do not mark
   Phase 12 complete.
