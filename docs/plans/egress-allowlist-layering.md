...
platform path bytes. Store the canonical display path in a sibling `path` metadata file for status
and collision checking; an existing key recording another path is an error.
...
`VHRN_MODE_FILE` points at the run mode. Reject commas in the resolved state root.
...
Return `PolicyRun` holding the lease and bind it before later fallible work. Owner cleanup is
lock-free: rename to `.<id>.dead` and remove it. `Drop` and SIGTERM share an idempotent path. Create
`ProxyGuard` after `PolicyRun` so the proxy stops first. Status/live-mode operations skip
post-discovery `NotFound` but propagate other errors.
...
Add `vhrn_state_from(home, xdg_state)` and `vhrn_state(home)` beside cache helpers. Harness state,
session databases, disposable config, tools layers, and gitconfig stay in cache.
...
project user layers. Existing `[net]` config no longer affects policy or mode.
...
harnesses, ordering, guarded/open variants, spaces, and quotes; every exit removes its run.
...
project identity, exact isolation, non-directory/mismatch errors, global/project live behavior,
same-scope concurrent writes, independent projects, atomic deny, remaining provenance, wrapper flag
ephemerality, empty first use, ignored legacy inputs, XDG/comma handling, lease/race cleanup,
idle-only log truncation/modes/symlink replacement, mount arguments, and guide quoting.
...
1. Confirm legacy cache and `[net]` have no effect.
...
The implementation agent owns this one-off operation; the user performs no manual migration or
cleanup. It is not product behavior, a script, or CI. Inspected paths:
...
7. Launch plain Claude and Codex. Verify new state, isolated built-ins, persistent global remainder,
   empty project layers, and absent old directory. Report carried/redundant/invalid/deleted entries.
...
- Repository-owned vhrn configuration.
