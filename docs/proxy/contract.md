...
sole public path is `/etc/vhrn/allowlist`. An empty item in a plural list
terminates startup. `VHRN_MODE_FILE` defaults to `/etc/vhrn/mode`,
`VHRN_PROXY_LISTEN` to `:8080`, and `VHRN_DENY_LOG` to no file output.
...
unreadable, or malformed required layer, or a missing/unreadable mode file,
denies and records the request with effective mode `enforce`.
...
lowercases, and removes one final dot. A domain entry permits itself and
dot-separated subdomains only. An IPv4 entry permits only that exact address.
`domain-policy.tsv` fixes these outcomes.
...
private, unspecified, link-local, multicast, 100.64.0.0/10, and the
IPv4-translated IPv6 prefix. Mapped IPv4 is classified as its IPv4 value. The
selected first permitted answer is dialed as a numeric address; no second name
lookup occurs. `ip-addresses.tsv` fixes the address cases and mixed-answer
outcome.
...
`Proxy-Authorization`, every standard hop-by-hop header, and every field named
by `Connection`. The same sanitization is applied to origin responses before
they reach the client. A denied public or local request returns 403. An origin
or connector failure returns 502.

For public HTTP, an origin-flushed first chunk reaches the client before origin
...
writing 200. Both public and local paths relay HTTP-parser-buffered bytes after
upgrade. A revoked grant blocks later requests and tunnels but does not end an
established local tunnel.
...
SIGTERM closes active connections and tunnels and produces a successful process
exit within one second in the bounded lifecycle probe. Disconnect cancellation
follows the connection closure behavior above.
